...
def rnn_step_backward(dnext_h, cache):
    ...
    # Derivative of tanh(z) is (1 - tanh^2(z)) = (1 - next_h^2)
    dtanh = dnext_h * (1 - next_h ** 2)
    
    # Gradients for input and weights
    dx = dtanh @ Wx.T
    dprev_h = dtanh @ Wh.T
    dWx = x.T @ dtanh
    dWh = prev_h.T @ dtanh
...
def rnn_backward(dh, cache):
    ...
    db = torch.zeros(Wx.shape[1], dtype=dh.dtype, device=dh.device)
    
    # Initialize dprev_h
    dprev_h = torch.zeros(N, H, dtype=dh.dtype, device=dh.device)
    
    # Backpropagate through time (BPTT) - go backward through time
...
class WordEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ######################################################################
        # x has shape (N, T) with integer indices
        # W_embed has shape (vocab_size, embed_size)
        # out should have shape (N, T, embed_size)
        out = self.W_embed[x]
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            # For attention LSTM, reshape features to (N, H, 4, 4) and project
            N = image_features.shape[0]
            A = self.image_to_A(image_features.permute(0, 2, 3, 1))  # (N, 4, 4, H)
            A = A.permute(0, 3, 1, 2)  # (N, H, 4, 4)
            h0 = A.mean(dim=(2, 3))  # (N, H)
        else:
            # For RNN/LSTM, pool the features and project to h0
            image_features_pooled = image_features.mean(dim=(2, 3))  # (N, C)
            h0 = self.image_to_h0(image_features_pooled)  # (N, H)
        
        # (2) Embed input captions
        captions_embed = self.word_embed(captions_in)  # (N, T, W)
        
        # (3) Pass through RNN/LSTM
        if self.cell_type == "rnn":
            h = self.rnn(captions_embed, h0)  # (N, T, H)
        elif self.cell_type == "lstm":
            h = self.lstm(captions_embed, h0)  # (N, T, H)
        elif self.cell_type == "attn":
            h = self.attn_lstm(captions_embed, A)  # (N, T, H)
        
        # (4) Project hidden states to vocabulary scores
        scores = self.score_proj(h)  # (N, T, V)
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "attn":
            # Project image features for attention LSTM
            A = self.image_to_A(image_features.permute(0, 2, 3, 1))  # (N, 4, 4, H)
            A = A.permute(0, 3, 1, 2)  # (N, H, 4, 4)
            h_t = A.mean(dim=(2, 3))  # (N, H)
            c_t = h_t.clone()
        else:
            # Pool and project for RNN/LSTM
            image_features_pooled = image_features.mean(dim=(2, 3))  # (N, C)
            h_t = self.image_to_h0(image_features_pooled)  # (N, H)
            
            if self.cell_type == "lstm":
                c_t = torch.zeros_like(h_t)
        
        # Initialize first word as START token
        prev_word = images.new(N).fill_(self._start).long()
        
        # Generate captions
        for t in range(max_length):
            # (1) Embed the previous word
            word_embed = self.word_embed(prev_word.unsqueeze(1))  # (N, 1, W)
            word_embed = word_embed.squeeze(1)  # (N, W)
            
            # (2) Make an RNN step
            if self.cell_type == "rnn":
                h_t = self.rnn.step_forward(word_embed, h_t)
            elif self.cell_type == "lstm":
                h_t, c_t = self.lstm.step_forward(word_embed, h_t, c_t)
            elif self.cell_type == "attn":
                attn, attn_weights = dot_product_attention(h_t, A)
                h_t, c_t = self.attn_lstm.step_forward(word_embed, h_t, c_t, attn)
                attn_weights_all[:, t, :, :] = attn_weights
            
            # (3) Get scores for vocabulary
            scores = self.score_proj(h_t)  # (N, V)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Compute activations from input and hidden state
        # Wx has shape (D, 4H), Wh has shape (H, 4H), b has shape (4H,)
        a = x @ self.Wx + prev_h @ self.Wh + self.b  # shape (N, 4H)
        
        # Split into 4 gates (each of shape (N, H))
        i = torch.sigmoid(a[:, :H])          # input gate
        f = torch.sigmoid(a[:, H:2*H])       # forget gate
        o = torch.sigmoid(a[:, 2*H:3*H])     # output gate
        g = torch.tanh(a[:, 3*H:])           # candidate cell state
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Flatten spatial dimensions of A for attention computation
    # A: (N, H, 4, 4) -> (N, H, 16)
    A_flat = A.reshape(N, H, -1)  # (N, H, 16)
    
    # Compute dot product: prev_h (N, H) @ A_flat (N, H, 16) -> (N, 16)
    # We use batched matrix multiplication
    # prev_h: (N, H, 1) @ A_flat transposed: (N, 1, 16) = (N, 16)
    dot_product = torch.bmm(
        prev_h.unsqueeze(2).transpose(1, 2),  # (N, 1, H)
        A_flat.transpose(1, 2)  # (N, 16, H)
    )  # (N, 1, 16)
    dot_product = dot_product.squeeze(1)  # (N, 16)
    
    # Scale by sqrt(H)
    dot_product = dot_product / (H ** 0.5)
    
    # Apply softmax to get attention weights
    attn_weights_flat = F.softmax(dot_product, dim=1)  # (N, 16)
    
    # Reshape back to (N, 4, 4)
    attn_weights = attn_weights_flat.reshape(N, 4, 4)
    
    # Compute weighted sum of A to get attention output
    # attn_weights_flat: (N, 16) @ A_flat: (N, H, 16).T -> (N, H)
    attn = torch.bmm(
        A_flat,  # (N, H, 16)
        attn_weights_flat.unsqueeze(2)  # (N, 16, 1)
    ).squeeze(2)  # (N, H)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Compute activations from input, hidden state, and attention
        # Wx: (D, 4H), Wh: (H, 4H), Wattn: (H, 4H), b: (4H,)
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        c_t = c0
        for t in range(T):
            # Compute attention for this timestep
            attn, _ = dot_product_attention(h_t, A)
            
            # LSTM step with attention
