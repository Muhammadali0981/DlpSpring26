...
def prepocess_input_sequence(
    ...
    for token in tokens:
        if token in spc_tokens:
            # It's a special token, add it directly
            out.append(token_dict[token])
        else:
            # It's a number, break it down to digits
            for digit in token:
...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        # Inner loop to compute dot products with all keys
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        # Compute scaled dot products: query[n] @ key[n].T
        # query[n]: (K, M), key[n]: (K, M) -> (K, M) @ (M, K) = (K, K)
        weights = torch.matmul(query[n], key[n].t()) / (M ** 0.5)
        
        # Apply softmax to get attention weights
        weights = F.softmax(weights, dim=1)  # softmax over key dimension
        
        # Compute weighted sum: weights @ values
        # weights: (K, K), value[n]: (K, M) -> (K, M)
        out[n] = torch.matmul(weights, value[n])
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    # Compute scaled dot products: query @ key.T
    # query: (N, K, M), key: (N, K, M) -> (N, K, K)
    ...
    # Compute weighted sum: weights @ values
    # weights_softmax: (N, K, K), value: (N, K, M) -> (N, K, M)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # Create list of SelfAttention layers
        self.heads = nn.ModuleList(
            [SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)]
        )
        
        # Create final linear layer to project concatenated output back to dim_in
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # Initialize scale (gamma) as ones and shift (beta) as zeros
    ...
    def forward(self, x: Tensor):
        ...
        # Compute variance and standard deviation (without torch.std)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        # First linear layer: inp_dim -> hidden_dim_feedforward
        self.linear1 = nn.Linear(inp_dim, hidden_dim_feedforward)
        
        # Second linear layer: hidden_dim_feedforward -> inp_dim (output has same shape as input)
        self.linear2 = nn.Linear(hidden_dim_feedforward, inp_dim)
        
        # Initialize weights for first linear
        c1 = (6 / (inp_dim + hidden_dim_feedforward)) ** 0.5
        nn.init.uniform_(self.linear1.weight, -c1, c1)
        
        # Initialize weights for second linear
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # MultiHead Attention block with residual connection
...
def get_subsequent_mask(seq):
    ...
    # Create a mask where upper triangular part (future) is masked
    # Shape: (K, K)
    subsequent_mask = torch.triu(torch.ones(K, K), diagonal=1).bool()
    
    # Expand to batch size: (N, K, K)
    mask = subsequent_mask.unsqueeze(0).expand(N, -1, -1)
    
    # Move mask to the same device as the input sequence
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_out = self.attention_self(dec_inp, dec_inp, dec_inp, mask=mask)
        self_attn_out = self.dropout(self_attn_out)
        dec_out = self.norm1(dec_inp + self_attn_out)
        
        # Cross-attention with encoder output
        cross_attn_out = self.attention_cross(dec_out, enc_inp, enc_inp)
        cross_attn_out = self.dropout(cross_attn_out)
        dec_out = self.norm2(dec_out + cross_attn_out)
        
        # Feed-forward network
        ff_out = self.feed_forward(dec_out)
        ff_out = self.dropout(ff_out)
        y = self.norm3(dec_out + ff_out)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Create 1D tensor with values n/K for n in [0, K)
    positions = torch.arange(K, dtype=torch.float32) / K
    
    # Replicate M times to get shape (K, M)
    y = positions.unsqueeze(1).expand(K, M)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Create position indices: (K, 1)
    positions = torch.arange(K, dtype=torch.float32).unsqueeze(1)
    
    # Create dimension indices for the sinusoidal formula
    # For even dimensions: sin(pos / 10000^(2i/M))
    # For odd dimensions: cos(pos / 10000^(2i/M))
    i = torch.arange(M, dtype=torch.float32).unsqueeze(0)
    
    # Compute the frequency: 10000^(2i/M)
    # But we need to handle even and odd indices separately
    # So we use: 10000^(2*floor(i/2)/M)
    div_term = 10000.0 ** ((2 * (i // 2)) / M)
    
    # Compute the sinusoid
    # For even indices (i=0,2,4,...): sin(pos / div_term)
    # For odd indices (i=1,3,5,...): cos(pos / div_term)
    pe = torch.zeros(K, M)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        mask = get_subsequent_mask(ans_b[:, :-1])  # Shape: (N, K-1, K-1)
        
        # Pass through decoder
        dec_out = self.decoder(a_emb_inp, enc_out, mask)
        
        # Reshape output to (N*O, V) where O is the output sequence length
        N, O, V = dec_out.shape
